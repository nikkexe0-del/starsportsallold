...
const ChannelMiniCard = memo(function ChannelMiniCard({
  channel,
  searchTerm = "",
  onPlay,
}: {
...
/* ─── virtualised grid using a sentinel + progressively revealed rows ── */
const PAGE_SIZE = 60; // channels rendered per page
...
function ChannelGrid({
    ...
    const io = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting) {
          setVisible((v) => Math.min(v + PAGE_SIZE, channels.length));
        }
      },
      { rootMargin: "300px" }
    );
  ...
  const slice = channels.slice(0, visible);
...
const Index = () => {
  ...
  const handlePlay = useCallback((ch: Channel) => navigate(`/channel/${ch.id}`), [navigate]);
